defmodule TimesheetWeb.TaskController do
  ...
  def index(conn, _params) do
    ...
    workers = Workers.list_manager_id(conn.assigns[:current_user].id)
    render(conn, "index.html", tasks: tasks, workers: workers)
  end

  def timesheetStatus(conn, _params) do
    tasks = Tasks.list_tasks()
    render(conn, "timesheetStatus.html", tasks: tasks)
  end
  ...
  def update(conn, %{"id" => id, "task" => task_params}) do
    task = Tasks.get_task!(id)
    IO.inspect("inside task update")
    task_params = Map.put(task_params, "approved", :true)
    IO.inspect(task_params)
